...
from encompass import compile as encompass_compile
...
def extract(text):
    if m := re.search(r"```python\s*(.*?)\s*```", text, re.DOTALL | re.I):
        return m.group(1).strip()
    if m := re.search(r"```\s*(.*?)\s*```", text, re.DOTALL):
        return m.group(1).strip()
...
async def main():
    ...
    out.exists() and __import__("shutil").rmtree(out)
    ...
    for f in sorted(src.rglob("*.java")):
        ...
        if code:
            (out / f"{f.stem}.py").write_text(code)
