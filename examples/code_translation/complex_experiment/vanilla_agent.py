...
def extract(text, lang="python"):
    if m := re.search(rf"```{lang}\s*(.*?)\s*```", text, re.DOTALL | re.I):
    ...
    for m in re.finditer(r"(public|private)?\s*\w+\s+(\w+)\s*\([^)]*\)\s*\{", code):
        ...
        for i, c in enumerate(code[start:], start):
            if c == "{":
                ...
                depth -= 1
                if depth == 0:
                    methods.append((name, code[start : i + 1]))
                    break
    return methods
    ...
    async def step(self):
        if self.fs is None:
            if self.idx >= len(self.files):
                return True
            self.fs = FileState(java=self.files[self.idx])
            return False
        ...
        if fs.step == 0:
            ...
            fs.score = 0.5 if "class " in fs.skeleton else 0.0
            fs.step = 1
            return False

        if fs.ms is None and fs.idx < len(fs.methods):
            n, c = fs.methods[fs.idx]
            fs.ms = MethodState(name=n, code=c)
            return False
        ...
        if fs.ms:
            ...
            if ms.step == 0:
                ...
                ms.py = extract(r)
                ms.score = 0.5 if "def " in ms.py else 0.0
                ms.step = 1
            elif ms.step == 1:
                await llm.generate(f"Tests:\n```python\n{ms.py}\n```", max_tokens=512)
                ms.step = 2
            elif ms.step == 2:
                await llm.generate(f"Java:\n```java\n{ms.code}\n```", max_tokens=512)
                ms.step = 3
            elif ms.step == 3:
                await llm.generate(f"Py:\n```python\n{ms.py}\n```", max_tokens=512)
                ms.step = 4
            elif ms.step == 4:
                ms.score += 0.8
                fs.full += f"\n\n{ms.py}"
...
async def main():
    ...
    dst.exists() and __import__("shutil").rmtree(dst)
