...
from benchmarks.gsm8k import DATASET as GSM8K_DATASET

# Import benchmarks
from benchmarks.gsm8k import create_math_agent, solver_sampler
from benchmarks.gsm8k_full import load_gsm8k_dataset
from benchmarks.reflexion import create_reflexion_llm_sampler, reflexion_agent, reflexion_sampler
from benchmarks.run_gsm8k_ollama import create_llm_sampler
from encompass.llm.ollama import OllamaModel
from runtime.engine import ExecutionEngine
from search.strategies import BFS, DFS, MCTS, BeamSearch, BestFirstSearch, BestOfNSearch
...
def get_strategy(name, store, engine, sampler, **kwargs):
    if name == "beam":
        return BeamSearch(
            ...
            diversity_penalty=kwargs.get("diversity", 0.0),
        )
    elif name == "mcts":
...
async def run(args):
    ...
    if args.benchmark == "gsm8k":
        ...
        if args.real_llm:
            ...
            print(
                "WARNING: Running gsm8k_full without --real-llm will likely fail as mock sampler only knows mini-eval problems."
            )
    ...
    print(
        f"--- Running {args.benchmark} ({len(tasks)} tasks) with {args.strategy} (Seed={args.seed}) ---"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--benchmark", type=str, required=True, choices=["gsm8k", "gsm8k_full", "arc", "reflexion"]
    )
    parser.add_argument(
        "--strategy",
        type=str,
        required=True,
        choices=["beam", "mcts", "best_of_n", "bfs", "dfs", "befs"],
    )
    ...
    parser.add_argument(
        "--real-llm", action="store_true", help="Use real LLM (Ollama) instead of mock sampler"
    )
    parser.add_argument(
        "--model", type=str, default="mistral", help="Ollama model name (if --real-llm is set)"
    )
    parser.add_argument(
        "--temperature", type=float, default=0.7, help="LLM temperature (default: 0.7)"
    )
    parser.add_argument(
        "--limit", type=int, default=None, help="Limit number of problems (for gsm8k_full)"
    )
