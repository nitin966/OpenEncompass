...
async def validate_search():
    ...
    print("Running beam search (width=5)...")
    ...
    for i, node in enumerate(terminal[:5]):
        print(
            f"    {i + 1}. Score: {node.score:6.0f}, Result: {node.metadata.get('result', 'N/A')}"
        )

    # Verify search actually explored alternatives
    unique_scores = {n.score for n in terminal}
    print(f"\n✓ Search explored {len(unique_scores)} different outcomes")
    print("  (Proves multiple paths were tried and compared)")
...
async def validate_replay():
    ...
    print("  (Should be fast - O(1) per step, not O(n))")
...
async def validate_scoring():
    ...
    print("Sampler provided: 1-10")
    print("Beam width: 3 (should keep top 3)")
    print("\nTop scoring paths:")
    for i, node in enumerate(terminal[:5]):
