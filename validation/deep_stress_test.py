...
class DeepSearchBenchmark:
    ...
    async def run_pathfinding_test(self):
        ...
        strategy = BeamSearch(store=store, engine=engine, sampler=grid_sampler, width=self.width)
    ...
    async def run_optimization_test(self):
        ...
        strategy = BeamSearch(store=store, engine=engine, sampler=number_sampler, width=self.width)

        print("Running search (this may take a while for large depth)...")
...
async def main():
    ...
    parser.add_argument(
        "--test",
        choices=["pathfinding", "optimization", "both"],
        default="pathfinding",
        help="Test type",
    )
    ...
    print("  ✓ O(1) replay mechanism")
    print("  ✓ Memory efficiency")
    print("  ✓ Search scales to production depth")
