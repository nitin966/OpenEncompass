...
async def demonstrate_search():
    ...
    async def tracking_sampler(node, metadata=None):
        ...
        options = [1, 2, 3]
        explored.append({"depth": node.depth, "trace": node.trace_history})
        return options
    ...
    print("  (Not just one greedy path!)")
...
checks = {
    ...
    "Score-Based Selection": all(
        terminal[i].score >= terminal[i + 1].score for i in range(len(terminal) - 1)
    ),
