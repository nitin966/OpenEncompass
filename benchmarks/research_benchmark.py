...
class ResearchBenchmark:
    ...
    async def run_gsm8k(self, model: str = "mistral", num_problems: int = 20, 
                        strategy: str = "beam", width: int = 3) -> Dict:
        """
        ...
        print(f"\n{'='*80}")
        print(f"GSM8K Benchmark - Research Protocol")
        print(f"{'='*80}")
        print(f"Model: {model}")
        ...
        for i, problem in enumerate(problems):
            ...
            try:
                ...
                if terminal_nodes:
                    ...
                    if is_solved:
                        print(f"✓ SOLVED in {elapsed:.2f}s ({len(nodes)} nodes, depth {result.search_depth})")
                    else:
    ...
    def _analyze_results(self, results: List[BenchmarkResult], problems: List[Dict]) -> Dict:
        ...
        return {
            ...
            'efficiency': {
                ...
                'avg_nodes_per_solved': statistics.mean([r.nodes_explored for r in solved_results]) if solved_results else 0,
    ...
    def _bootstrap_ci(self, data: List[bool], n_bootstrap: int = 1000, confidence: float = 0.95) -> tuple:
        """
        Bootstrap confidence interval for accuracy.
    ...
    def generate_report(self) -> str:
        ...
        report = f"# Benchmark Results - Research Protocol\n\n"
    ...
    def _report_gsm8k(self) -> str:
        ...
        report = f"## GSM8K (Math Word Problems)\n\n"
        ...
        report += f"| 95% CI (Bootstrap) | [{r['ci_95']['lower']:.1%}, {r['ci_95']['upper']:.1%}] |\n"
        ...
        if r.get('by_difficulty'):
            report += f"### Performance by Difficulty\n\n"
            report += f"| Difficulty | Problems | Solved | Accuracy |\n"
            report += f"|------------|----------|--------|----------|\n"
            for diff in ['easy', 'medium', 'hard']:
        ...
        report += f"- **Throughput**: {r['efficiency']['problems_per_second']:.2f} problems/second\n"
        report += f"- **Avg nodes per solved problem**: {r['efficiency']['avg_nodes_per_solved']:.1f}\n"
