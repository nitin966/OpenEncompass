...
class BenchmarkSuite:
    ...
    async def run_comparison(self, benchmarks: List[str], models: List[str], num_problems: int = 20):
        """
    ...
    def generate_report(self) -> str:
        ...
        if 'comparison' in self.results:
            ...
            report += f"### GSM8K Results\n\n"
            report += f"| Model | Accuracy | 95% CI | Avg Time | Avg Nodes |\n"
            report += f"|-------|----------|--------|----------|----------|\n"
    ...
    def save_results(self):
        ...
        results_file = self.output_dir / f"results_{self.model_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        with open(results_file, 'w') as f:
            json.dump(self.results, f, indent=2)
        
        # Save markdown report
        report = self.generate_report()
        report_file = self.output_dir / f"report_{self.model_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
...
async def main():
    ...
    parser.add_argument("--suite", default="gsm8k", choices=["gsm8k", "humaneval", "all", "comparison"])
